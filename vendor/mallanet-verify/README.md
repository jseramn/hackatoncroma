...
- `DATABASE_URL` — optional. Postgres role that can DML `verify` only. If omitted, the server uses the in-memory seed (`verify-operator-001`). Persistencia real solo con Neon.
- `psql` once, to apply SQL when using Neon
...
Missing `CROMA_API_KEY` aborts **before** tools register. Missing `DATABASE_URL` falls back to the in-memory seed. Reload MCP after edits.

Or: `npm start` (`tsx src/index.ts`).
